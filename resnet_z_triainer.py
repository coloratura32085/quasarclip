...
def main():
    # 加载数据集
    train_dataset = load_from_disk(f"{ROOT_PATH}/data_g3_z/train_dataset")  # 替换为你的数据集路径
    val_dataset = load_from_disk(f"{ROOT_PATH}/data_g3_z/test_dataset")
    ...
    logger = TensorBoardLogger(save_dir=f'{ROOT_PATH}/outputs/image_pre/resnet18_z/logs', name="resnet_z")

    # 初始化 Trainer
    trainer = Trainer(max_epochs=100, accelerator="gpu", devices=8,
                      callbacks=[checkpoint_callback],strategy="ddp",logger = logger)
