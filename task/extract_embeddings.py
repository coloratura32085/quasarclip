...
class FlattenAndReshape:
    ...
    def __call__(self, x):
        ...
        # 使用动态的 cropsize 来计算 reshape 的维度
        reshape_dim = self.cropsize
        x_reshaped = x_flat.view(reshape_dim, reshape_dim, 5)
...
def main():
    ...
    # 路径设置
    parser.add_argument('--train_data', type=str, default=f'../data/data_g3_z/train_dataset')
    parser.add_argument('--test_data', type=str, default=f'../data/data_g3_z/test_dataset')
    ...
    if args.mode == 'image':
        ...
        transform = transforms.Compose([
            CustomCenterCrop(size=args.cropsize),
            FlattenAndReshape(cropsize=args.cropsize),
        ])
    ...
    for split_name, data_path in datasets.items():
        ...
        out_path = os.path.join(output_dir, out_name)  # 使用修改后的output_dir
