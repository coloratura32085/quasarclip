...
# 结果保存路径
reconstruction_file_path = f'{ROOT_PATH}/data/testPre/reconstruction.npy'
embedding_file_path = f'{ROOT_PATH}/data/testPre/embedding.npy'
...
for batch_idx, spectra_batch in enumerate(tqdm(data_loader, desc="Processing batches", ncols=100)):
    ...
    # 每个批次处理完后，将保存部分结果
    if (batch_idx + 1) % 1000 == 0 or batch_idx == len(data_loader) - 1:  # 每 1000 个批次保存一次
        # 处理并保存 reconstruction 数据
        if reconstruction_list:
            # 如果文件已存在，读取并追加
            if os.path.exists(reconstruction_file_path):
                existing_reconstruction = np.load(reconstruction_file_path)
                existing_reconstruction = np.concatenate([existing_reconstruction, np.concatenate(reconstruction_list, axis=0)], axis=0)
            else:
                existing_reconstruction = np.concatenate(reconstruction_list, axis=0)
            np.save(reconstruction_file_path, existing_reconstruction, allow_pickle=False)
        ...
        if embedding_list:
            ...
            if os.path.exists(embedding_file_path):
                existing_embedding = np.load(embedding_file_path)
                existing_embedding = np.concatenate([existing_embedding, np.concatenate(embedding_list, axis=0)], axis=0)
            else:
                existing_embedding = np.concatenate(embedding_list, axis=0)
            np.save(embedding_file_path, existing_embedding, allow_pickle=False)
            embedding_list = []  # 清空列表
            existing_embedding = []  # 清空 new_embedding

# 最终输出
print(f"Reconstruction saved to {reconstruction_file_path}")
print(f"Embedding saved to {embedding_file_path}")
