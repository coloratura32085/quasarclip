...
from imageutil.engine_pretrain import train_one_epoch
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default=f'{ROOT_PATH}/data/train_dataset', type=str,
                        help='dataset path')
...
def main(args):
    ...
    dataset_train = load_from_disk(args.data_path)
    dataset_train = dataset_train['image']
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        current_loss = train_stats['loss']  # Assuming 'loss' is returned in train_stats
        if current_loss < best_loss:
            best_loss = current_loss
            print(f"New best loss: {best_loss:.4f}, saving model...")
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, filename="best_loss.pth")
